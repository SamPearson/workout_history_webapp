from flask import Flask
from flask_sqlalchemy import SQLAlchemy
from os import path
from flask_login import LoginManager
import sys
db = SQLAlchemy()
DB_NAME = "database.db"


def create_app():
    app = Flask(__name__)
    #todo: FLEX-2 - move this so the key won't be in github
    app.config['SECRET_KEY'] = 'opieupsmadfa4j3rlemfapoq83japefwoewsfjapo4ifafj'
    app.config['SQLALCHEMY_DATABASE_URI'] = f'sqlite:///{DB_NAME}'
    db.init_app(app)

    from .views.allviews import views
    from .views.auth import auth
    from .views.admin import admin_blueprint

    #to be removed, broken into many files
    app.register_blueprint(views, url_prefix='/')

    app.register_blueprint(auth, url_prefix='/')
    app.register_blueprint(admin_blueprint, url_prefix='/')

    from .models import User, Note, WorkoutPhase

    create_database(app)

    login_manager = LoginManager()
    login_manager.login_view = 'auth.login'
    login_manager.init_app(app)

    @login_manager.user_loader
    def load_user(id):
        return User.query.get(int(id))

    return app

def create_database(app):
    if not path.exists('website/' + DB_NAME):
        print('Could not locate database, attempting to create')
        db.create_all(app=app)
        print('Database created successfully')
    else:
        print('Database located')
